...
from onyx.skills.scheduled_skills import ScheduledCRUDSkill
from onyx.config import get_skill_config
from onyx.util import play_mp3

class AlarmSkill(ScheduledCRUDSkill):
    def __init__(self):
        super(AlarmSkill, self).__init__("AlarmSkill", None)
        self.config_alarm = get_skill_config('alarm_skill', 'alarm')
        self.alarm_on = False
        self.max_delay = int(self.config_alarm.get('AlarmSkill', 'max_delay'))
        self.repeat_time = int(self.config_alarm.get('AlarmSkill', 'repeat_time'))
        self.extended_delay = int(self.config_alarm.get('AlarmSkill', 'extended_delay'))
        self.file_path = join(dirname(__file__), self.config_alarm.get('AlarmSkill', 'filename'))

    def initialize(self):
        super(AlarmSkill, self).initialize()
        intent = IntentBuilder('AlarmSkillStopIntent')\
            .require('AlarmSkillStopVerb')\
